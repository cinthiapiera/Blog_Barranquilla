      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {filteredAttractions.length === 0 ? (
          ...
          filteredAttractions.map((attraction) => (
            <div
              key={attraction.id}
              className=" border bg-slate-100 bg-opacity-60 backdrop-blur-sm rounded-lg p-5 shadow-lg transition-all ease-in-out delay-100 duration-300 hover:scale-110 hover:z-10"
            >
              ...
              <Link
                to={`${attraction.id}`}
                className="mt-2 block text-cyan-500 hover:font-bold bottom-0"
              >
                See more...
              </Link>
            </div>
